...
int main(void) {
	...
	*    one line at a time, and filtering and deduping any lines that do not belong to the target patient or are duplicates  */
	...
	fgets(line, 1440, infile); //  This reads + discards the first line. 
	puts(line); // display the first line to screen
	puts("First line accessed !\n");

	while (fitbitArray[i].patient == "12cx7") {     // filter out everyone except target patient
		...
		i++;
	} //last line of csv data is  "23:59:00,0.968900025,0,0,68,0,1"  ( no "\n") 

	fclose(infile);

	/*   15 pts for correctly cleaning the data and parsing each record into the corresponding fields:
	*   lines not belonging to correct patient are ignored.
	*   duplicate lines should be deleted  (DO NEXT)
	*   5 pts for correctly converting the strings to the field types  (CHECK?)
	*/


	/*   5 pts for correctly storing the fields into the FitbitData array (below) */
	...
	double totalPoorSleep = badSleepArray[0].totalBadSleep;


	/*   8 pts for correctly formatting and outputting the data, without any duplicate entries, to “Results.csv” and to the screen   */
	write_to_results(*fitbitArray, outfile, totalkcal, totalDistance, totalFloors, totalSteps, aveHR, maxSteps, totalPoorSleep);
